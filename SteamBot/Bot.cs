...
namespace SteamBot
{
    public class Bot : IDisposable
    {
        ...
        public Bot(Configuration.BotInfo config, string apiKey, UserHandlerCreator handlerCreator, bool debug = false, bool process = false)
        {
            ...
            repeatCheck = new System.Timers.Timer();
            repeatCheck.Elapsed += new ElapsedEventHandler(processNextQue);
            repeatCheck.Interval = 5000; // 1000 ms is one second
        ...
        public TradeOffer NewTradeOffer(SteamID other)
        {
            return tradeOfferManager.NewOffer(other);
                ...
                // processNextQue executes the next offer in que 
        public void processNextQue(object source, ElapsedEventArgs e)
        {
            //stop the timer
            repeatCheck.Stop();
            starsDB database = new starsDB();

            bool lottoStatus = (database.lottoStatus());
            //only do this if lotto has ended
            if (lottoStatus == true)
            {
                if (checkDistroRecieveComplete() == false)
                {
                    repeatCheck.Start();
                    return;
                }
                ...
                {
                    Console.WriteLine("Recieved all items for distro, beginning distro process and approve start of next lotto");
                    string storageBot = "76561197979168247";
                    string lottoID = database.getCurrentLotto();
                    string winnerID = database.decideLotteryWinner(lottoID);
                    double totalPot = 0;
                    int numberOfItems = 0;
                    database.getLottoTotal(lottoID, out totalPot, out numberOfItems);
                    Console.WriteLine("Winner Found: " + winnerID);
                    //failed to grab winner ID restart timer
                    if (winnerID.Equals("0000"))
                    {
                        Console.WriteLine("failed to grab winner ID");
                        repeatCheck.Start();
                        return;
                    }

                    //begin building cut list 
                    double percentageCut;
                    database.getPercentCut(out percentageCut);
                    Console.WriteLine("Percentage Cut: " + percentageCut);

                    //build cut item list for collecting ourselves before distributing rest to the winner
                    List<string> cutList = database.buildCutList(lottoID, percentageCut, totalPot);
                    //temp set to 76561197979168247 = doctorn, will change later to match bank bot 
                    //cutlist test output
                    for (int i = 0; i < cutList.Count; i++)
                    {
                        Console.WriteLine("Cutlist Item: " + cutList[i]);
                    }

                    if (database.setNewOwnerCut(cutList, storageBot) == false)
                    {
                        Console.WriteLine("Failed to set new owner id for cut list ");
                        //failed to set new owner id 
                        return;
                    }
                    //build the winner list based off what isn't in the cut list 
                    List<string> winnerList = database.buildWinnerList(cutList, lottoID);
                    if (database.setNewOwnerCut(winnerList, winnerID) == false)
                    {
                        Console.WriteLine("failed to assign new owner to winner list items");
                        return;
                    }
                    //condition check if the winnerList was built properly
                    if (winnerList == null || winnerList.Count <= 0)
                    {
                        Console.WriteLine("failed to build winner Item List");
                        //die if the list wasn't built
                        return;
                    }

                    //grab the descriptionIDs of the current List 
                    List<string>[] descList = database.getdescriptionIDList(lottoID);
                    if (descList == null || descList[0].Count <= 0)
                    {
                        Console.WriteLine("failed to get descriptionID list");
                        return;
                    }

                    //load distroBot backpack to grab item IDS
                    List<long> contextId = new List<long>();
                    contextId.Add(2);
                    //access bot backpack and assign item ID to all items that are ready to be distributed 
                    mySteamInventory.load(730, contextId, SteamClient.SteamID);
                    foreach (var item in mySteamInventory.items)
                    {
                        //assign variables of inventory status
                        int currentAppID = item.Value.appid;
                        var currentContextID = item.Value.contextid;
                        //asset ID is the item ID 
                        long currentAssetID = Convert.ToInt64(item.Value.assetid);
                        var currentDescID = item.Value.descriptionid;

                        for (int i = 0; i < descList[0].Count; i++)
                        {
                            //check if desc id matches the current item being searched for in the backpack 
                            if (descList[0][i].Equals(currentDescID) && descList[1][i].Equals("0000")) 
                            {
                                Console.WriteLine("desc id: " + currentDescID + " asset ID:" + currentAssetID);
                                //assign the item an item ID for trading 
                                descList[1][i] = currentAssetID.ToString();
                                break;
                            }
                        }
                    }

                    for (int i = 0; i < descList[1].Count; i++)
                    {
                        Console.WriteLine("debug test " + descList[1][i] + " hash ID " + descList[2][i]);
                    }

                    if (database.assignItemIDToList(descList) == false)
                    {
                        //kill on item assign fail
                        Console.WriteLine("failed to assign item id to items");
                        //return;
                    }
                    try
                    {
                        //send offer to winner and distro storage
                        //storage
                        sendTradeOffer(cutList, storageBot, "5nE3wxrP", lottoID, false);
                        //winner
                        sendTradeOffer(winnerList, winnerID, "5nE3wxrP", lottoID, true);
                    }
                    catch
                    {
                        //report to admin
                        Console.WriteLine("failed to send offer to winner/cut bot please check manually ");
                        database.sendMail("Error failed to complete distro lotteryID" + lottoID);
                    }
            ...
            {
                //otherwise start timer again and keep checking for end of lotto and distro collection complete
                repeatCheck.Start();
            }
        }
        //updateWinner sets distro to complete and updates the winner ID on live lotto
        public bool updateWinner(string winnerID)
        {
            starsDB database = new starsDB();
            bool result = database.updateLiveLottoWinner(winnerID);
            return result;
        }
        public bool sendTradeOffer(List<string> itemList, string ownerSID, string tradeToken, string lotteryID, bool winner)
        {
            starsDB database = new starsDB();
            List<long> contextId = new List<long>();
            contextId.Add(2);
            
            SteamID sendToSID = new SteamID();
            sendToSID.SetFromUInt64(Convert.ToUInt64(ownerSID));
            var itemsToSendOffer = this.NewTradeOffer(sendToSID);
            string tradeOfferID;
            foreach (var item in mySteamInventory.items)
            {
                //assign variables of inventory status
                ...
                var currentDescID = item.Value.descriptionid;
                //current asset id
                //looping compare the asset ID of backpack and the current item to find the items we are xfering to distro bot
                for (int i = 0; i < itemList.Count; i++)
                {
                    string itemID = database.getItemID(itemList[i]);

                    if (currentAssetID.Equals(Convert.ToInt64(itemID)))
                    {
                        //Console.WriteLine("found match " + currentAppID + " " + currentContextID + " " + currentAssetID);
                        //add item to distro bot offer list
                        Console.WriteLine("sending to " + ownerSID + " " + currentAppID + " " + currentContextID + " " + currentAssetID + " ");
                        itemsToSendOffer.Items.AddMyItem(currentAppID, currentContextID, currentAssetID, 0);
                        break;
                    }
                }
            }
            //send the created offer to distro bot
            if (itemsToSendOffer.Items.NewVersion)
            {
                string message = "";
                if (winner == true)
                {
                    message = "Congrats you won the CS GO Stars Lottery # " + lotteryID + "!!!!! Thanks for using csgostars.net";
                }
                else
                {
                    message = "Sending cut offer to storage for lottery ID " + lotteryID;
                }
                if (itemsToSendOffer.SendWithToken(out tradeOfferID, tradeToken, message))
                {
                    if (winner == true)
                    {
                        Console.WriteLine("sent trade offer to winner " + tradeOfferID);
                    }
                    else
                    {
                        Console.WriteLine("sent trade offer to cut storage " + tradeOfferID);
                    }
        ...
        public bool checkDistroRecieveComplete()
        {
            starsDB database = new starsDB();
            string lottoID = database.getCurrentLotto();
            double totalValue = 0;
            int numberOfItems = 0;
            //get the values for comparing 
            if (database.getLottoTotal(lottoID, out totalValue, out numberOfItems) == false)
            {
                return false;
            }
            if (database.checkIfCollectionComplete(lottoID, numberOfItems, totalValue))
            {
                return true;
            }
            return false;
